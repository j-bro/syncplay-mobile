...
**`room/ui/chat/`** - `RoomSectionChat` (renders only when `supportsChat`; caps at `roomFeatures.maxChatMessageLength`, never strips backslashes; `SelectionContainer` on desktop), `MessageRow` (two message shapes with grouping and a time gutter; peer image URLs auto-render - untrusted, a known ledger item), `GifPanel` (Klipy search/trending/recents with a failure state and shimmer tiles), `RoomFadingChatLayout` (transient messages while the HUD is hidden; honours font/outline/shadow prefs and PiP).
