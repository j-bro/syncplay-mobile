...
- Identity applies in AGP `finalizeDsl` (AFTER module DSL), so the exoOnly applicationId swap lives in the root block (`id(if (exoOnly) …) { }`), NOT in androidApp's `defaultConfig`. A module-level override cannot win.
- Runtime constants are generated into `shared/commonMain` by `kiteConfig { buildConfig { } }` as the public object `SyncplayMobile.shared.KiteBuildConfig` (the default class name since 1.0.0; it no longer clashes with AGP's `BuildConfig`). `IS_DEBUG` is detected per invocation from the requested task names or the CocoaPods configuration property; anything ambiguous is NOT debug. Never add a field called `DEBUG`: Xcode defines `DEBUG=1` and the exported header stops compiling.
- versionCode scheme is `1 | major(3) | minor(3) | patch(2) | rebuild(1)` (0.24.0 is `1000024000`, 0.23.2 is `1000023020`). Every code is larger than the old 2.x code for the same version, so Play monotonicity holds. Bump `android { rebuild }` / `ios { rebuild }` for a re-upload instead of faking a patch release. The release workflow computes the same scheme when it stamps the pbxproj.
- Branding is on-demand: launcher assets regenerate ONLY via `kiteRewriteLogo`, which installs both Android and iOS assets transactionally and, because `logo { rewrite { replaceOld = true } }` is set, takes over known legacy Android collisions after backing them up. `compileSdk >= 33` makes it emit `mipmap-anydpi-v33` wrappers with the themed-icon `<monochrome>` layer (issue #143).
- Tasks: `kiteDoctor` (diagnose, never fails), `kiteCheck` (the release-grade gate; writes a JSON/SARIF report and fails on errors), `kitePlan` (which mutations are armed), `kiteVerify` (the resolved model), `kiteRewriteXcode` (pbxproj identity/version/knownRegions, armed by `ios { rewrite { } }`, run after every version bump), `kiteRewriteLogo` (armed by `logo { rewrite { } }`). Neither rewrite runs during a normal build. The `kiteInternal*` tasks are plugin internals.
- Plugin state: the ownership receipts under `**/.kiteconfig/` are COMMITTED on purpose (they prove which generated files the plugin owns); `.kiteconfig/recovery/`, `**/.kiteconfig/**/*.lock` and `*.kiteconfig.bak` are gitignored local state.
...
| File | Purpose |
|---|---|
| `AdamScreen.kt` | Root composable: per-screen ViewModels via `viewModelFactory`, `NavDisplay` backstack with a real `onBack`, CompositionLocals (`LocalGlobalViewmodel`, `LocalRoomViewmodel`, `LocalTheme`, `LocalScreen`, `LocalChatPalette`, `LocalRoomUiState`, `LocalPrefsState`, `LocalType`). Wrapped once in `GlassBackdrop`. `entryProvider` maps Screen→composable. |
...
| `Screen.kt` | `@Serializable sealed interface Screen : NavKey`: `Home`, `Room(joinConfig?)`, `ThemeCreator(themeToEdit?)`, `Settings(categoryKey?)`. Server hosting is no longer a screen; it lives inline on Home. |
...
| `PlatformCallback.kt` | Platform ops interface: PiP, brightness, device volume (`deviceVolumeSteps`/`get`/`setDeviceVolume`), haptics, media session, server service, language, shortcuts, `launchSystemFilePicker`, `copyText`, `shareText`. |

### Home (`commonMain/app/home/`)

`HomeScreen.kt` - one adaptive form of four blocks (identity, server, engine, join) placed by the window. Width: two columns from 640dp (each column at least 284dp, the narrowest at which the three-engine picker keeps every word), else one column capped at 420dp. Height, in tiers that scale with the text size up to 1.5x: short (under 400dp under the bar) pins two independently scrolling columns, pairs the identity fields in one row when a column has 280dp per text scale, and tightens spacing; compact (under 600dp, 700dp in Custom or Host) tightens spacing and shrinks the engine cells to 98dp; otherwise roomy. One column is a `SpreadColumn`: gaps grow from 24dp to at most 72dp and what is left centres the form, so a tall window never stretches it. The two-column block is centred, and the right column's minimum height is the left column's capped at the viewport, so a tall hosting panel never pushes Join off screen. The form pads only the horizontal and bottom safe-drawing insets, because the bar has already paid the status bar. Help under a field shows only while it is focused or failing. Fields are the drawn `Field` control. The server block is a segmented choice: Official (`syncplay.pl`, port row 8995-8999), Custom, or Host mine (hosting panel inline, joins `127.0.0.1`). The engine block is `HomeEnginePicker` (a segmented frame showing every engine with a badge; the "?" under the selection unfolds a per-engine story card; the row takes the height of its tallest cell so large text grows it). Join is the one gradient key, with the shortcut saver as its own key beside it, hidden on desktop where its hook is a no-op; username/room are stripped of backslashes, trimmed and capped (149/34) in the shared join builder. `HomeGolden` (desktopTest) renders the form at device sizes minus their system bars and fails when a label is cut or the join key leaves the screen at rest. `HomeViewmodel.kt` - `joinRoom(config?)`; null = solo mode. `JoinConfig.kt` - `@Serializable user/room/ip/port/pw`; defaults ip `syncplay.pl`, port 8997; `save()` only when `REMEMBER_INFO`. Components: `HomeTopBar` (wordmark + logo→About, theme picker, settings glyph → `Screen.Settings`), `PopupAPropos` (tagline, facts, Watch-alone row via `joinRoom(null)`, source/report links; the story sits beside the links on a window under 480dp tall so a phone on its side sees every action without scrolling; `AboutBody` is the frame-less body that `AboutGolden` renders), `PopupDidYaKnow` (first-launch tips with a working Next; `NEVER_SHOW_TIPS`).

`HomeViewmodel.updateCheck` retains an explicit update check across About popup closures. `UpdateCheckController` admits one request at a time and caches successful results; failures can be retried. `UpdateCheckAction` renders the current version as a green, non-clickable live status, or a newer release as an accent action opening its release page. `Attributions.kt` names KitePlayer and KiteFFmpeg.
...
Room startup publishes `PlayerManager.player` and `isPlayerReady` after awaiting the previous
engine's teardown, then starts the network connection. Initial server messages must never run
before that publication. While waiting, the room keeps its artwork, chat, notices and Leave
action; player tools, gestures, settings, playlist actions, PiP and reconnect stay unavailable.
Android resume/subtitle callbacks and public seek entry points also check readiness. A delayed
startup regression (`RoomStartupTest`) renders the real room and leaves before a player exists.
...
**`room/ui/chat/`** - `RoomSectionChat` (renders only when `supportsChat`; caps at `roomFeatures.maxChatMessageLength`, never strips backslashes; `SelectionContainer` on desktop), `MessageRow` (two message shapes with grouping and a time gutter; peer image URLs auto-render - untrusted, a known ledger item), `GifPanel` (Klipy search/trending/recents with a failure state and shimmer tiles), `RoomFadingChatLayout` (transient messages while the HUD is hidden, stacked under the notices on the centre line rather than at the left edge; honours font/outline/shadow prefs and PiP).

**`room/ui/rightcards/`** - `RoomSidePanels`/`RoomSectionSlidingCards` (animated dock container) hosting `CardUserInfo` (two roster views: Compact and Expanded, with readiness dots, self tint and controller edge/star; legacy `files` preferences show Expanded), `CardSharedPlaylist` (rows + header keys that unfold option strips; empty-state hints), `CardRoomPrefs` (the settings console body via `roomSettings()`, engine rows folded into the player category), `CardTracks` (audio+subtitle columns, load-from-file and search rows first), `CardGestures`, `CardSeekTo` (one timecode field), `CardAddMedia` (route sheet with the link form inline).

`UserRosterPanel` is the roster body with explicit data/callback inputs, so desktop renders exercise the real panel. A small animated icon in the header switches Compact/Expanded; this card never repeats the room user count or shows a tab strip. Expanded rows always show the full filename, duration, readable file size and file-match status; filenames start at 12sp and autosize toward two lines down to 11sp, then grow at that readable floor; metadata also uses 12sp with tighter line spacing. Compact rows show username and a width-measured head/ellipsis/tail filename (`abbreviateRosterFileName`, because native middle ellipsis differs by platform), removing balanced leading release groups and recognized media extensions via `compactRosterFileName`; enlarged text stacks the two fields. Only a tapped peer shows mute/report and operator readiness actions, one peer at a time; a tapped compact row also reveals full file details. Peer position is retained in the protocol model but no longer displayed. Mute is local session chat filtering; report opens a prefilled GitHub issue.

**`room/ui/tabs/`** - `RoomRail` (the room's actions: panels, lock, PiP, managed room, leave; actions fold behind More once per session; drawn cells with semantics), `RoomUnlockableLayout`, `PopupManagedRoom` (one modal, segmented create-or-identify; sends `WireMessage.controllerAuth`).

**`room/ui/misc/`** - `RoomTransportKeys` (translucent play key + jump keys; the custom skip sits between back and forward), `RoomGestureInterceptor` (double-taps accumulate into ONE seek per 900 ms chain; 600 ms long-press previews and commits once; swipe brightness left / volume right on the `VolumeLadder`; real `systemGestures`/`displayCutout`/`waterfall` edge guards with an 8%-height floor, all read via `rememberUpdatedState` because rotation restarts nothing), `RoomGestureValueHud` (the readout), `RoomPlayButton` (renders and decides from collected `isNowPlaying`; readiness-gated `controlPlayback`; buffering rounds the key to a circle and walks its gradient, nothing grows under it, so the transport never shifts), `RoomBackgoundArtwork` (filename typo lives on). **`room/ui/statinfo/`** - `RoomSectionStatusInfo` (room name + user count, distinct Connecting/Reconnecting/Disconnected labels, SxxExx tag; hidden in solo). **`room/models/`** - `Message` (BiDi-isolated chat rendering), `MessagePalette`.
...
`OSD_SAME_ROOM` defaults off: routine same-room events remain in the chat log without showing
notices over video unless enabled. Existing saved choices are preserved.

`InlineColorPage` reserves the preview/reset footer, then sizes KolorPicker from the remaining
height and derives its width (10.5:8, keeping the main colour field square). The page switches
the panel's scrolling off; category/colour lists keep the host scroll. Short panels omit the
redundant explanation beneath their title, and small pickers proportionally shrink their thumb
and track padding. Inline save/reset callbacks use the app ViewModel scope, since opening the editor disposes
the original settings row. Leaving flushes the final debounced selection; opacity survives
subsequent colour changes. Reset restores both the stored default and the visible picker,
while initialization callbacks do not overwrite the follow-theme sentinel.

### Theme (`commonMain/app/theme/`)

`Tokens.kt` - the design tokens: `Space` (6dp ladder), `Radius`, `Motion` (all tweens collapse under reduced motion), `Type` (five roles, provided as `LocalType`), `Palette` (semantic colours derived per theme via MaterialKolor; `LocalPalette` / `palette`; `overVideo()` pins the room dark), `Tier` (`Flat`/`Panel`/`Chrome`/`Scrim` surface tiers). `BuiltinThemes.kt` - default `TRINITY` ("Violet"); also `DAYLIGHT` (light), `PYNCSLAY` ("Neon"), `GrayOLED`, `ALLEY_LAMP`; stored copies of renamed themes migrate. `SaveableTheme` (serializable; lazy `dynamicScheme`). `ThemePicker` (list rows with edit/delete on their own targets; delete asks first), `ThemeMiniature` (themes preview as a miniature of the app), `ThemeCreatorScreen` (live canvas, token controls, single-write save via `replaceTheme`). `Theming.kt` - brand `NeoSP1/2/3` from the Trinity build constants; `flexibleGradient` returns the active theme's raw seeds and is consumed only by the logo and wordmark.

### UI components (`commonMain/app/uicomponents/`)

`controls/` - the drawn control set with semantics built in: `Rocker` (toggle), `ScrubTrack` (slider with ticks, buffered band, progress semantics), `Stepper`, `Segmented` (radio-group semantics), `Field` (hairline text field with a name), `GlyphButton`, `ListRow`, `Actions`, `Tag`, `Timecode`, `HelpTip`, `Chevron`, `Swatch`, `Progress`, `Rule`, `Text` (auto-size capable), `Glyphs` (the drawn glyph set). `frames/` - `ScreenFrame`, `Modal` (the ONE modal/popup frame: Ask/Panel/full sizes plus a bottom sheet; scrolls, `imePadding`, Escape; a panel is 92 percent of the window capped at 440dp, or 720dp when the window is under 480dp tall, since a phone on its side has height to lose and width to spare; the cap is one layout step because `fillMaxWidth(f).widthIn(max)` never caps; the action row is a `FlowRow`, so three keys at large text wrap instead of squeezing the last one to a letter per line), `PanelFrame`, `Notice` (`NoticeQueue`, max 3 stacked; `NoticeHost` renders with live-region semantics). `GlassSurface.kt` - `Modifier.surface(tier, shape)` is the only glass entry; `GlassBackdrop` captures on demand (`GlassDemand` counter), the room supplies its own capture over the video layer via `glassBackdropLayer`; `DISABLE_FROSTED_GLASS` switches the whole system to solid panels. Also `SynkplayLogo` (canvas mark, Oklab-tuned dark lead-in), `Wordmark`, `WidthClass`, `AnimatedImage` (expect/actual, `onLoaded`), `MessagePalette`, `PopupMediaDirs`.

`Text(autoSize = FontSizeRange(min, max))` shrinks a label toward the floor and ellipsises only past it. Foundation auto-size on its own never shrinks under `TextOverflow.Ellipsis`, because the cut text counts as fitting, so `Segmented`, `Tag`, the engine cells and the shortcut saver all go through the wrapper. The wordmark is sized in dp, not sp: the system text scale is for reading. `Actions` measures labels against the available width, autosizes within the label/group font range, prefers two lines and allows further wrapping at the minimum. Action containers have minimum heights and grow with their labels; paired About links stack at narrow widths or large font scales. List labels/values and group headings wrap without line caps, with fixed value columns scaling alongside the system font. `ScrubTrack` skips a zero-sized canvas and limits tick/playhead widths to the track, so collapsing room layouts cannot produce a negative clamp range. Desktop rendering regressions cover 0–8dp tracks in both directions, About actions at 280–360dp and 200% text, and expanded in-room setting explanations.
...
| `mediaResolver` | `NewPipeMediaResolver` (YT/SoundCloud/PeerTube/Bandcamp/MediaCCC), in `jvmShared` and shared with desktop | `YouTubeKitMediaResolver` (YT only; no-op if bridge unregistered) |
...
iOS Swift bridges (registered at startup in `iosApp/iOSApp.swift`): `instantiateSwiftNioNetworkManager`, `instantiateYouTubeKitBridge`. The Kotlin sides are abstract classes / nullable factory vars in `iosMain` whose `isAvailable` tracks registration.

SwiftNIO throws `NetworkManager.SocketGoneException().asError()` when no socket exists. Kotlin exports that exception as a nested Swift type; the former flattened name does not compile. Kotlin-only compilation does not validate the Swift bridges: build the Xcode workspace against the regenerated framework to check them.
...
- **What it is.** `KitePlayer` is a Compose video player from the `io.github.yuroyami` group on
  Maven Central; `KiteFFmpeg` is the FFmpeg-backed decoder inside it. Both are the owner's own
...
**Destroy contract (all Android engines + iOS VlcKitImpl/AVPlayerImpl):** flip `isInitialized=false` **first** (so every `isInitialized`-gated method refuses the engine), then cancel `playerSupervisorJob` (stops the position tracker, releases the `RoomViewmodel` graph), then release the native engine. mpv's process-global handle hard-crashes (`CHECK_MPV_INIT()`) if a tracker poll outlives teardown; its global observer list must be detached first.
...
**Key commands:** `./gradlew :desktopApp:run` (dev), `:desktopApp:createDistributable` / `packageDmg|Msi|Deb` (ship). `-PdebugProtocol=true` on any of these flips `DEBUG_SYNCPLAY_PROTOCOL` wire logging.
...
| `server/ui/ServerHostPanel.kt` | The hosting UI, inline on Home under the "Host mine" server choice: address block with copy and share, status row with evidence, config rows, the log list with a follow rule and an errors filter. |
...
The Android server process is kept alive by `SyncplayServerService` (a foreground notification only; logic runs in `ServerViewmodel`'s scope).

**Android media session.** `AndroidPlatformCallback` creates a `RoomMediaSessionPlayer` adapter and
hands its `MediaSession` to `SyncplayMediaSessionService` through `RoomMediaSessionHolder`.
Room entry uses `startService`, because the player is initially idle; Media3 promotes the service
to foreground when playback starts. The service explicitly adds the held session in
`onStartCommand`: a plain start intent does not call `onGetSession` (controller connections do).
Repeated starts refresh that registration, a missing room stops the service, and
`START_NOT_STICKY` prevents an orphan restart after process death. Service destruction detaches its
controllers; room teardown releases the session and stops the service through the application
context. Lock-screen and headset controls go through the room dispatcher, preserving readiness
and room-wide playback behavior.
