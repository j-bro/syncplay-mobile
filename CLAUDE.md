...
**`room/ui/misc/`** - `RoomTransportKeys` (translucent play key + jump keys), `RoomGestureInterceptor` (double-taps accumulate into ONE seek per 900 ms chain; 600 ms long-press previews and commits once; swipe brightness left / volume right on the `VolumeLadder`; real `systemGestures`/`displayCutout`/`waterfall` edge guards with an 8%-height floor, all read via `rememberUpdatedState` because rotation restarts nothing), `RoomGestureValueHud` (the readout), `RoomPlayButton` (renders and decides from collected `isNowPlaying`; readiness-gated `controlPlayback`), `RoomBackgoundArtwork` (filename typo lives on). **`room/ui/statinfo/`** - `RoomSectionStatusInfo` (room name + user count, distinct Connecting/Reconnecting/Disconnected labels, SxxExx tag; hidden in solo). **`room/models/`** - `Message` (BiDi-isolated chat rendering), `MessagePalette`.
